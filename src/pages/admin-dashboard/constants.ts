...
DEPT_NAMES.forEach((dept, deptIdx) => {
  generatedUsers.push({
    id: userId++,
    name: `Admin - ${dept.split(' ')[0]}`,
    email: `admin.${DEPT_CODES[deptIdx].toLowerCase()}@eduverse.edu`,
    role: 'admin',
    status: 'active',
    lastActive: '2026-02-03 08:00',
    department: dept,
  });
  PROFESSOR_NAMES[dept].forEach((name) => {
    generatedUsers.push({
      id: userId++,
      name,
      email: `${name.replace('Dr. ', '').toLowerCase().replace(/ /g, '.')}@eduverse.edu`,
      role: 'instructor',
      status: 'active',
      lastActive: '2026-02-03 09:00',
      department: dept,
    });
  });
  TA_NAMES[dept].forEach((name) => {
    generatedUsers.push({
      id: userId++,
      name,
      email: `${name.replace('Eng. ', '').toLowerCase().replace(/ /g, '.')}@eduverse.edu`,
      role: 'ta',
      status: 'active',
      lastActive: '2026-02-03 10:00',
      department: dept,
    });
  });
  for (let s = 1; s <= 10; s++) {
    generatedUsers.push({
      id: userId++,
      name: `Student ${DEPT_CODES[deptIdx]}-${s}`,
      email: `student${s}.${DEPT_CODES[deptIdx].toLowerCase()}@eduverse.edu`,
      role: 'student',
      status: 'active',
      lastActive: '2026-02-03 11:00',
      department: dept,
    });
  }
});
...
const COURSE_TEMPLATES: Record<
  string,
  { code: string; name: string; credits: number; prereqs: string[] }[]
> = {
  'Computer Science and Engineering': [
...
DEPT_NAMES.forEach((dept) => {
  ...
  COURSE_TEMPLATES[dept].forEach((tmpl, i) => {
    generatedCourses.push({
      ...
      capacity: 150,
      status: 'active',
      instructor: profs[i % profs.length].name,
      instructorId: profs[i % profs.length].id,
      taIds: [tas[i % tas.length].id, tas[(i + 1) % tas.length].id],
      prerequisites: tmpl.prereqs,
    });
  });
});
...
export const DEPARTMENTS = DEPT_NAMES.map((name, i) => ({
  ...
  name,
  faculty: 'Engineering',
  head: PROFESSOR_NAMES[name][0],
  courses: 6,
  students: 150,
  instructors: 6,
  tas: 8,
  budget: `$${(2 + Math.random() * 2).toFixed(1)}M`,
}));
  ...
  ['Computer Science and Engineering', 'Electrical Engineering'].forEach((dept) => {
    ...
    dc.forEach((c, i) => {
      s.push({
        ...
        courseCode: c.code,
        courseName: c.name,
        instructor: pr[i % pr.length].name,
        instructorId: pr[i % pr.length].id,
        day: days[i % days.length],
        startTime: `${String(9 + (i % 4)).padStart(2, '0')}:00`,
        endTime: `${String(10 + (i % 4)).padStart(2, '0')}:30`,
      ...
      s.push({
        ...
        courseCode: c.code,
        courseName: c.name,
        instructor: ta[i % ta.length].name,
        instructorId: ta[i % ta.length].id,
        day: days[(i + 2) % days.length],
        startTime: `${String(11 + (i % 3)).padStart(2, '0')}:00`,
        endTime: `${String(12 + (i % 3)).padStart(2, '0')}:30`,
        room: rooms[(i + 3) % rooms.length],
        type: (i % 2 === 0 ? 'lab' : 'section') as const,
        department: dept,
      });
    });
  });
...
export const ENROLLMENT_PERIODS = [
  {
    ...
    description: 'Fall 2025 course registration window',
  },
  {
    id: 2,
  ...
  },
  {
    ...
    description: 'Spring 2026 — students select their courses (prerequisites enforced)',
  },
  {
    id: 4,
    department: 'Electrical Engineering',
  ...
  },
  {
    ...
    description: 'Spring 2026 registration — opens soon',
  },
];
    ...
    generatedCourses
      ...
      .forEach((c, i) => {
        e.push({
          ...
          date: `2025-10-${String(20 + i).padStart(2, '0')}`,
          startTime: '09:00',
          endTime: '11:00',
          room: rooms[i % rooms.length],
          duration: 120,
...
export const SUPPORT_TICKETS = [
  ...
  },
  {
    ...
    subject: 'Grade submission error',
    priority: 'high',
    status: 'inProgress',
  ...
  },
  {
    id: 3,
    user: 'Fatima Khalil',
    subject: 'Request for course enrollment',
    priority: 'medium',
    status: 'pending',
...
export const AUDIT_LOGS = [
  {
    id: 1,
    action: 'User Created',
    user: 'admin@eduverse.edu',
    target: 'new.user@eduverse.edu',
  ...
  },
  {
    ...
    user: 'sarah.martinez@eduverse.edu',
    target: 'CS101',
  ...
  },
  {
    ...
    action: 'Role Changed',
    user: 'admin@eduverse.edu',
    target: 'mohamed.ali@eduverse.edu',
  ...
  },
  {
    ...
    user: 'admin@eduverse.edu',
    target: 'layla.ahmed@eduverse.edu',
    timestamp: '2026-01-15 09:45',
    ip: '192.168.1.100',
  },
];
...
export const NOTIFICATION_TEMPLATES = [
  {
    ...
    content: 'Dear {{name}}, Welcome to EduVerse platform...',
  ...
  },
  {
    ...
    content: 'You have been enrolled in {{courseName}}...',
  ...
  },
  {
    ...
    content: 'The system will be under maintenance on {{date}}...',
  ...
  },
  {
    ...
    content: 'Your grade for {{assignment}} has been posted...',
...
export const GAMIFICATION_SETTINGS = {
  ...
  badges: [
    ...
    },
    {
      ...
      description: 'Complete a course with 90%+',
      icon: '🏆',
      enabled: false,
    },
  ],
...
export const API_INTEGRATIONS = [
  {
    ...
    name: 'Moodle',
    status: 'connected',
    lastSync: '2026-02-03 08:00',
    syncFrequency: 'Every 6 hours',
    dataTypes: ['courses', 'grades', 'users'],
  },
  {
    id: 2,
    name: 'Blackboard',
  ...
  },
  {
    ...
    status: 'connected',
    lastSync: '2026-02-03 06:00',
    syncFrequency: 'Daily',
    dataTypes: ['assignments', 'students'],
  },
  {
    id: 4,
...
export const RECENT_ACTIVITY = [
  ...
  },
  {
    ...
    description: 'CS303 - Advanced Algorithms added',
    time: '5 hours ago',
  },
  {
    type: 'system',
  ...
  },
  {
    ...
    description: 'CPU usage peaked at 85%',
    time: '1 day ago',
  },
  {
    type: 'integration',
    title: 'Moodle Sync Completed',
    description: '256 records synchronized',
    time: '1 day ago',
  },
];
