...
import {
  Settings, Shield, Plug, Briefcase, Server,
  Globe, Lock, Mail, MessageSquare, Bell,
  CreditCard, Video, Code, Bug, Save,
  RotateCcw, Key, Clock, Webhook
} from 'lucide-react';
...
export function SettingsHubPage() {
  ...
  const inputClass = `w-full rounded-lg border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-rose-500 ${
  ...
  const Toggle = ({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) => (
    <button
      ...
      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
        checked ? 'bg-rose-600' : isDark ? 'bg-gray-600' : 'bg-gray-300'
      }`}
    >
      <span
        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
          checked ? (isRTL ? 'translate-x-1.5' : 'translate-x-6') : (isRTL ? 'translate-x-6' : 'translate-x-1')
        }`}
      />
    </button>
  );
      ...
      <div className={cardClass}>
        ...
        <div className="divide-y divide-gray-700/20">
          <SettingRow label="Current Semester">
            <select value={settings.currentSemester} onChange={e => update('currentSemester', e.target.value)} className={selectClass}>
          ...
          <SettingRow label="Semester Start Date">
            <input type="date" value={settings.semesterStart} onChange={e => update('semesterStart', e.target.value)} className={inputClass} />
          </SettingRow>
          <SettingRow label="Semester End Date">
            <input type="date" value={settings.semesterEnd} onChange={e => update('semesterEnd', e.target.value)} className={inputClass} />
          </SettingRow>
        </div>
      </div>
      ...
      <div className={cardClass}>
        ...
        <div className="divide-y divide-gray-700/20">
          <SettingRow label="Open Registration">
            <Toggle checked={settings.openRegistration} onChange={v => update('openRegistration', v)} />
          </SettingRow>
          <SettingRow label="Registration Deadline">
            <input type="date" value={settings.registrationDeadline} onChange={e => update('registrationDeadline', e.target.value)} className={inputClass} />
          </SettingRow>
          <SettingRow label="Max Credits Per Student">
            <input type="number" value={settings.maxCredits} onChange={e => update('maxCredits', Number(e.target.value))} className={inputClass} />
          </SettingRow>
        </div>
      </div>
      ...
      <div className={cardClass}>
        ...
        <div className="divide-y divide-gray-700/20">
          <SettingRow label="Institution Name">
            <input type="text" value={settings.institutionName} onChange={e => update('institutionName', e.target.value)} className={inputClass} />
          </SettingRow>
          <SettingRow label="Primary Color">
            <div className="flex items-center gap-2">
              <input type="color" value={settings.primaryColor} onChange={e => update('primaryColor', e.target.value)} className="h-9 w-9 rounded border-0 cursor-pointer" />
              <span className={`text-sm font-mono ${subTextClass}`}>{settings.primaryColor}</span>
            </div>
          </SettingRow>
          <SettingRow label="Logo URL">
            <input type="text" value={settings.logoUrl} onChange={e => update('logoUrl', e.target.value)} placeholder="https://..." className={inputClass} />
          </SettingRow>
        </div>
      </div>
      ...
      <div className={cardClass}>
        ...
        <div className="divide-y divide-gray-700/20">
          ...
          <SettingRow label="Allow User Language Override">
            <Toggle checked={settings.allowLanguageOverride} onChange={v => update('allowLanguageOverride', v)} />
          </SettingRow>
        </div>
      </div>
    </div>
      ...
      <div className={cardClass}>
        ...
        <div className="divide-y divide-gray-700/20">
          <SettingRow label="Minimum Length">
            <input type="number" value={settings.minPasswordLength} onChange={e => update('minPasswordLength', Number(e.target.value))} className={inputClass} />
          </SettingRow>
          <SettingRow label="Require Uppercase">
            <Toggle checked={settings.requireUppercase} onChange={v => update('requireUppercase', v)} />
          </SettingRow>
          <SettingRow label="Require Numbers">
            <Toggle checked={settings.requireNumbers} onChange={v => update('requireNumbers', v)} />
          </SettingRow>
          <SettingRow label="Require Special Characters">
            <Toggle checked={settings.requireSpecialChars} onChange={v => update('requireSpecialChars', v)} />
          </SettingRow>
          <SettingRow label="Password Expiry (days)">
            <input type="number" value={settings.passwordExpiry} onChange={e => update('passwordExpiry', Number(e.target.value))} className={inputClass} />
          </SettingRow>
        </div>
      </div>
      ...
      <div className={cardClass}>
        ...
        <div className="divide-y divide-gray-700/20">
          <SettingRow label="Require 2FA for Admins">
            <Toggle checked={settings.require2FAAdmins} onChange={v => update('require2FAAdmins', v)} />
          </SettingRow>
          ...
          <SettingRow label="2FA Method">
            <select value={settings.twoFAMethod} onChange={e => update('twoFAMethod', e.target.value)} className={selectClass}>
              <option>App</option>
              <option>SMS</option>
              <option>Email</option>
            </select>
      ...
      <div className={cardClass}>
        ...
        <div className="divide-y divide-gray-700/20">
          <SettingRow label="Session Timeout (minutes)">
            <input type="number" value={settings.sessionTimeout} onChange={e => update('sessionTimeout', Number(e.target.value))} className={inputClass} />
          </SettingRow>
          <SettingRow label="Max Concurrent Sessions">
            <input type="number" value={settings.maxConcurrentSessions} onChange={e => update('maxConcurrentSessions', Number(e.target.value))} className={inputClass} />
          </SettingRow>
      ...
      <div className={cardClass}>
        ...
        <div className="flex items-center justify-between">
          <p className={`text-sm ${textClass}`}>
            <span className="font-semibold text-rose-500">5</span> users currently blocked
          </p>
          <button className="px-4 py-2 text-sm font-medium rounded-lg bg-rose-600 text-white hover:bg-rose-700 transition-colors">
      ...
      <div className={cardClass}>
        ...
        <div className="divide-y divide-gray-700/20">
          <SettingRow label="SMTP Server">
            <input type="text" value={settings.smtpServer} onChange={e => update('smtpServer', e.target.value)} className={inputClass} />
          </SettingRow>
          ...
          <SettingRow label="Sender Email">
            <input type="text" value={settings.senderEmail} onChange={e => update('senderEmail', e.target.value)} className={inputClass} />
          </SettingRow>
        </div>
        <div className="mt-4">
          <button className="px-4 py-2 text-sm font-medium rounded-lg bg-rose-600 text-white hover:bg-rose-700 transition-colors">
      ...
      <div className={cardClass}>
        ...
        <div className="divide-y divide-gray-700/20">
          <SettingRow label="Provider">
            <select value={settings.smsProvider} onChange={e => update('smsProvider', e.target.value)} className={selectClass}>
              <option>Twilio</option>
              <option>Vonage</option>
            </select>
          </SettingRow>
          <SettingRow label="API Key">
            <input type="password" value={settings.smsApiKey} onChange={e => update('smsApiKey', e.target.value)} className={inputClass} />
          </SettingRow>
      ...
      <div className={cardClass}>
        ...
        <div className="divide-y divide-gray-700/20">
          <SettingRow label="Firebase Enabled">
            <Toggle checked={settings.firebaseEnabled} onChange={v => update('firebaseEnabled', v)} />
          </SettingRow>
          <SettingRow label="Server Key">
            <input type="password" value={settings.firebaseServerKey} onChange={e => update('firebaseServerKey', e.target.value)} className={inputClass} />
          </SettingRow>
        </div>
      </div>
      ...
      <div className={cardClass}>
        ...
        <div className="space-y-3">
          {settings.webhooks.map((wh, i) => (
            <div key={i} className={`flex flex-col sm:flex-row sm:items-center gap-2 p-3 rounded-lg ${isDark ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
        ...
        <div className="mt-4">
          <button
            onClick={() => update('webhooks', [...settings.webhooks, { name: 'New Webhook', url: '', active: false }])}
            className="px-4 py-2 text-sm font-medium rounded-lg bg-rose-600 text-white hover:bg-rose-700 transition-colors"
          >
            Add Webhook
          </button>
        </div>
  ...
  const renderBusiness = () => (
    <div className="space-y-6">
      ...
      <div className={cardClass}>
        ...
        <div className="divide-y divide-gray-700/20">
          ...
          <SettingRow label="Stripe API Key">
            <input type="password" value={settings.stripeApiKey} onChange={e => update('stripeApiKey', e.target.value)} className={inputClass} />
          </SettingRow>
      ...
      <div className={cardClass}>
        ...
        <div className="divide-y divide-gray-700/20">
          <SettingRow label="Provider">
            <select value={settings.videoProvider} onChange={e => update('videoProvider', e.target.value)} className={selectClass}>
              <option>Zoom</option>
              <option>Teams</option>
              <option>Meet</option>
            </select>
          ...
          <SettingRow label="API Key">
            <input type="password" value={settings.videoApiKey} onChange={e => update('videoApiKey', e.target.value)} className={inputClass} />
          </SettingRow>
          <SettingRow label="Auto-Create Meetings">
            <Toggle checked={settings.autoCreateMeetings} onChange={v => update('autoCreateMeetings', v)} />
          </SettingRow>
        </div>
      </div>
    </div>
  ...
  const renderSystem = () => (
    <div className="space-y-6">
      ...
      <div className={cardClass}>
        ...
        <div className="divide-y divide-gray-700/20">
          <SettingRow label="Rate Limit (per hour)">
            <input type="number" value={settings.rateLimit} onChange={e => update('rateLimit', Number(e.target.value))} className={inputClass} />
          </SettingRow>
          <SettingRow label="API Version">
            <input type="text" value={settings.apiVersion} onChange={e => update('apiVersion', e.target.value)} className={inputClass} />
          </SettingRow>
      ...
      <div className={cardClass}>
        ...
        <div className="divide-y divide-gray-700/20">
          <SettingRow label="Debug Mode" description="⚠️ Enabling debug mode may expose sensitive data">
            <Toggle checked={settings.debugMode} onChange={v => update('debugMode', v)} />
          </SettingRow>
          <SettingRow label="Verbose Logging">
            <Toggle checked={settings.verboseLogging} onChange={v => update('verboseLogging', v)} />
          </SettingRow>
        </div>
        <div className="mt-4">
          <p className={`text-xs font-semibold uppercase tracking-wider mb-3 ${subTextClass}`}>Feature Flags</p>
          <div className="space-y-3">
            <div className="flex items-center justify-between">
      ...
      <div className={cardClass}>
        ...
        <div className="divide-y divide-gray-700/20">
          <SettingRow label="Log Level">
            <select value={settings.logLevel} onChange={e => update('logLevel', e.target.value)} className={selectClass}>
              <option>Info</option>
          ...
          <SettingRow label="Retention (days)">
            <select value={settings.logRetention} onChange={e => update('logRetention', e.target.value)} className={selectClass}>
      ...
      <div className={cardClass}>
        ...
        <div className="divide-y divide-gray-700/20">
          ...
          <SettingRow label="Latest Version">
            <span className={`text-sm font-mono text-rose-500`}>{settings.latestVersion}</span>
          </SettingRow>
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-gray-900' : 'bg-gray-50'} ${isRTL ? 'rtl' : 'ltr'}`}>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 pb-24">
        ...
        {/* Section Tabs */}
        <div className={`flex gap-1 overflow-x-auto mb-8 border-b ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
      ...
      {/* Save Bar */}
      {hasChanges && (
        <div className={`fixed bottom-0 inset-x-0 z-50 border-t ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} shadow-lg`}>
          <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={handleSave}
                className="flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg bg-rose-600 text-white hover:bg-rose-700 transition-colors"
              >
