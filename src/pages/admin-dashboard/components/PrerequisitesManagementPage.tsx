...
export function PrerequisitesManagementPage({
  courses,
  adminDepartment,
  onUpdatePrerequisites,
}: PrerequisitesManagementPageProps) {
  ...
  const filteredCourses = deptCourses.filter(
    (c) =>
      c.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      c.code.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const coursesWithPrereqs = deptCourses.filter(
  ...
  const removePrerequisite = (courseId: number, prereqCode: string) => {
    ...
    onUpdatePrerequisites(
      courseId,
      course.prerequisites.filter((p) => p !== prereqCode)
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          {
            label: t('currentPrerequisites'),
            value: coursesWithPrereqs,
            icon: GitBranch,
            color: 'text-blue-500',
          },
          {
      ...
      >
        <div className="relative">
          ...
          <input
            type="text"
            placeholder={t('searchPrerequisites')}
            value={searchTerm}
        ...
        <div className="flex flex-wrap gap-3">
          {deptCourses
            ...
            .map((course) => (
              <div
                key={course.id}
                className={`flex items-center gap-2 px-3 py-2 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}
              >
                {course.prerequisites.map((prereq, i) => (
                  <React.Fragment key={prereq}>
                    <span
                      className={`text-sm font-medium px-2 py-1 rounded ${isDark ? 'bg-blue-900/30 text-blue-400' : 'bg-blue-100 text-blue-700'}`}
                    >
                      {prereq}
                    </span>
                ...
                <span
                  className={`text-sm font-semibold px-2 py-1 rounded ${isDark ? 'bg-blue-900/30 text-blue-400' : 'bg-blue-100 text-blue-700'}`}
                >
                  {course.code}
                </span>
              </div>
            ))}
          {deptCourses.filter((c) => c.prerequisites && c.prerequisites.length > 0).length ===
            0 && (
          ...
          return (
            ...
            >
              <button
                ...
                className={`w-full flex items-center justify-between p-4 hover:bg-opacity-50 transition-colors ${isDark ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}
              >
                <div className="flex items-center gap-3">
                  <span
                    className={`text-xs font-semibold px-2 py-1 rounded bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400`}
                  >
                    {course.code}
                  </span>
                      ...
                      {course.prerequisites &&
                        course.prerequisites.map((prereq) => {
                          ...
                          return (
                            <span
                              key={prereq}
                              className={`inline-flex items-center gap-1 text-sm px-3 py-1 rounded-full ${isDark ? 'bg-blue-900/30 text-blue-400' : 'bg-blue-100 text-blue-700'}`}
                            >
                              {prereq} {prereqCourse && `- ${prereqCourse.name}`}
                              <button
                                onClick={() => removePrerequisite(course.id, prereq)}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {available.slice(0, 10).map((c) => (
                        <button
                          key={c.id}
                          onClick={() => addPrerequisite(course.id, c.code)}
                          className={`inline-flex items-center gap-1 text-sm px-3 py-1 rounded-full border transition-colors ${isDark ? 'border-gray-600 text-gray-300 hover:bg-gray-700' : 'border-gray-200 text-gray-700 hover:bg-gray-50'}`}
                        >
