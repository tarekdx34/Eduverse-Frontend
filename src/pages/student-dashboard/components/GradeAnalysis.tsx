...
const recommendations = [
  {
    ...
    description: 'Your Discrete Mathematics grade is below average. Consider joining a study group or using online practice platforms.',
    action: 'Find Study Resources',
  },
  {
    title: 'Maintain Your Momentum',
    description: 'Your GPA has been steadily increasing. Keep up the great work in Software Engineering and Web Development.',
    action: 'View Study Plan',
  },
  {
    title: 'Explore Advanced Topics',
    description: 'Based on your strong performance in Machine Learning, consider taking Advanced AI or Deep Learning next semester.',
...
export const GradeAnalysis = () => {
  ...
  const getRankColor = (rank: number) => {
    switch (rank) {
      case 1: return 'from-amber-400 to-yellow-500 text-white';
      case 2: return 'from-slate-300 to-gray-400 text-white';
      case 3: return 'from-amber-600 to-orange-700 text-white';
      default: return isDark ? 'from-white/10 to-white/5 text-slate-400' : 'from-slate-100 to-slate-200 text-slate-500';
    }
  };
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      case 'high': return isDark ? 'text-red-400 bg-red-500/10 border-red-500/20' : 'text-red-600 bg-red-50 border-red-200';
      case 'medium': return isDark ? 'text-amber-400 bg-amber-500/10 border-amber-500/20' : 'text-amber-600 bg-amber-50 border-amber-200';
      case 'low': return isDark ? 'text-blue-400 bg-blue-500/10 border-blue-500/20' : 'text-blue-600 bg-blue-50 border-blue-200';
      default: return '';
    }
  };
      ...
      <div className="bg-gradient-to-br from-[var(--accent-color)] via-blue-600 to-blue-700 rounded-2xl p-8 text-white shadow-xl">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 bg-green-500/20 border border-green-400/30 px-4 py-2 rounded-full">
            ...
            <span className="text-green-200 font-semibold text-lg">+5%</span>
          </div>
        </div>
      </div>

      {/* Tab Navigation */}
      <div className={`${isDark ? 'bg-card-dark border border-white/5' : 'glass'} rounded-[2.5rem] flex gap-2 p-2`}>
        {tabs.map((tab) => {
          const Icon = tab.icon;
          return (
            <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="p-5 space-y-3">
              {gradeDistribution.map((item) => (
                <div key={item.grade} className="flex items-center gap-3">
                  <span className={`w-8 text-sm font-semibold text-right ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                    {item.grade}
                  </span>
                  <div className={`flex-1 ${isDark ? 'bg-white/5' : 'bg-slate-100'} rounded-full h-5 overflow-hidden`}>
                    {item.count > 0 && (
                      <div
                        className={`h-full bg-gradient-to-r ${item.color} rounded-full flex items-center justify-end pr-2 transition-all`}
                        style={{ width: `${(item.count / maxGradeCount) * 100}%`, minWidth: item.count > 0 ? '2rem' : '0' }}
                      >
          ...
          <div className={`${isDark ? 'bg-card-dark border border-white/5' : 'glass'} rounded-[2.5rem] overflow-hidden`}>
            ...
            <div className="p-5 grid grid-cols-2 gap-4">
              {[
                { label: 'Total Courses', value: '25', icon: BookOpen, color: 'text-blue-500', bg: isDark ? 'bg-blue-500/10' : 'bg-blue-50' },
                { label: 'Passed', value: '23', icon: CheckCircle, color: 'text-green-500', bg: isDark ? 'bg-green-500/10' : 'bg-green-50' },
                { label: 'Highest Grade', value: 'A+', icon: Star, color: 'text-indigo-500', bg: isDark ? 'bg-indigo-500/10' : 'bg-indigo-50' },
                { label: 'Lowest Grade', value: 'C', icon: AlertTriangle, color: 'text-amber-500', bg: isDark ? 'bg-amber-500/10' : 'bg-amber-50' },
              ].map((stat) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>{stat.value}</div>
          ...
          <div className={`${isDark ? 'bg-card-dark border border-white/5' : 'glass'} rounded-[2.5rem] overflow-hidden`}>
            ...
            <div className="p-5">
              <div className="flex items-end justify-between mb-4">
                <div>
                  <div className={`text-4xl font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>120</div>
              ...
              <div className={`flex justify-between mt-3 text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                <span>0</span>
                <span>36</span>
                <span>72</span>
                <span>108</span>
            ...
            <div className="p-5">
              <div className="flex items-end gap-3" style={{ height: '220px' }}>
                {semesterGPA.map((sem) => {
                  ...
                  return (
                    <div key={sem.semester} className="flex-1 flex flex-col items-center gap-2 h-full justify-end">
                      <span className={`text-xs font-semibold ${isDark ? 'text-white' : 'text-slate-700'}`}>
                        {sem.gpa.toFixed(2)}
                      </span>
                      <div
                      ...
                      <span className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'} text-center leading-tight`}>
                ...
                return (
                  <div key={sem.semester}>
                    <div className="flex items-center justify-between mb-1">
                      <span className={`text-sm font-medium ${isDark ? 'text-white' : 'text-slate-700'}`}>{sem.semester}</span>
                      <div className="flex items-center gap-2">
                        <span className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{sem.credits} cr</span>
                        <span className={`text-sm font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>{sem.gpa.toFixed(2)}</span>
                      </div>
                    </div>
          ...
          <div className={`${isDark ? 'bg-card-dark border border-white/5' : 'glass'} rounded-[2.5rem] overflow-hidden`}>
            ...
            <div className="p-4 space-y-3">
              {courseRanking.map((course) => (
                ...
                >
                  <div className={`w-9 h-9 rounded-lg bg-gradient-to-br ${getRankColor(course.rank)} flex items-center justify-center text-sm font-bold shadow-sm`}>
                    #{course.rank}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className={`text-sm font-medium truncate ${isDark ? 'text-white' : 'text-slate-800'}`}>{course.name}</div>
                    <div className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{course.code}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm font-bold ${isDark ? 'text-white' : 'text-slate-700'}`}>{course.percentage}%</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className={`${isDark ? 'bg-card-dark border border-white/5' : 'glass'} rounded-[2.5rem] overflow-hidden`}>
              ...
              <div className="p-4 space-y-3">
                {topPerformers.map((course) => (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${isDark ? 'bg-green-500/10' : 'bg-green-100'}`}>
                    ...
                    <div className="text-right">
                      ...
                      <div className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{course.percentage}%</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className={`${isDark ? 'bg-card-dark border border-white/5' : 'glass'} rounded-[2.5rem] overflow-hidden`}>
              ...
              <div className="p-4 space-y-3">
                {needsFocus.map((course) => (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${isDark ? 'bg-amber-500/10' : 'bg-amber-100'}`}>
                      <AlertTriangle className="w-5 h-5 text-amber-500" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className={`text-sm font-medium truncate ${isDark ? 'text-white' : 'text-slate-800'}`}>{course.name}</div>
              ...
              <div className="p-4 space-y-4">
                {studyGoals.map((goal) => {
                  ...
                  return (
                    <div key={goal.title}>
                      <div className="flex items-center justify-between mb-1">
                        <span className={`text-sm font-medium ${isDark ? 'text-white' : 'text-slate-700'}`}>{goal.title}</span>
            ...
            <div className={`${isDark ? 'bg-card-dark border border-white/5' : 'glass'} rounded-[2.5rem] overflow-hidden`}>
              ...
              <div className="p-4 space-y-3">
                {focusAreas.map((item) => (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className={`text-sm font-medium ${isDark ? 'text-white' : 'text-slate-800'}`}>{item.area}</div>
                      <div className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{item.reason}</div>
                    </div>
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium border capitalize ${getPriorityColor(item.priority)}`}>
                      {item.priority}
                    </span>
                  </div>
                ))}
