        ...
        <div className={`p-4 border-b ${isDark ? 'border-white/5' : 'border-slate-100'}`}>
          <div className="relative">
            ...
            {query && (
              <button
                onClick={() => setQuery('')}
                className={`absolute right-4 top-1/2 -translate-y-1/2 p-1 rounded-full transition-all ${isDark ? 'hover:bg-white/10' : 'hover:bg-slate-50'}`}
              >
                <X className={`w-5 h-5 ${isDark ? 'text-slate-500' : 'text-slate-500'}`} />
              </button>
            )}
          </div>
