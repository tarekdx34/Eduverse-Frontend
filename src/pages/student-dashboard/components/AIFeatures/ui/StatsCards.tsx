  ...
  const cards = [
    {
      ...
      sub: (
        <span className="text-green-500 text-xs font-bold flex items-center gap-1">
          <TrendingUp className="w-3 h-3" /> {isRTL ? 'جميعها متاحة' : 'All unlocked'}
        </span>
      ),
      icon: 'apps',
    ...
    },
    {
      ...
      sub: (
        <span className="text-emerald-500 text-xs font-bold">
          {mostUsedFeature.usageCount} {isRTL ? 'استخدام' : 'uses'}
        </span>
      ),
      icon: 'star',
    ...
    },
    {
      ...
      sub: (
        <span className="text-amber-500 text-xs font-bold flex items-center gap-1">
          <TrendingUp className="w-3 h-3" /> {isRTL ? '+8 ساعات هذا الأسبوع' : '+8h this week'}
        </span>
      ),
      icon: 'schedule',
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8" dir={isRTL ? 'rtl' : 'ltr'}>
      {cards.map((card, i) => (
        <div
          key={i}
          className={`p-6 glass rounded-[2.5rem] flex items-center justify-between group transition-all ${card.hoverBorder}`}
        >
          <div>
            ...
            <h4
              className={`mt-1 font-bold ${isDark ? 'text-white' : 'text-slate-800'} ${card.isText ? 'text-lg leading-tight' : 'text-3xl'}`}
            >
              {card.value}
            </h4>
          ...
          <div
            className={`w-12 h-12 rounded-xl flex items-center justify-center ${card.iconBg} transition-all`}
          >
