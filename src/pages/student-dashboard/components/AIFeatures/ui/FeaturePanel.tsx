    ...
    return (
      <div className="glass rounded-[2.5rem] sticky top-6 overflow-hidden">
        <div className="text-center py-20 px-6">
          ...
          <h4 className={`font-bold mb-2 ${isDark ? 'text-white' : 'text-slate-800'}`}>
            Ready to Begin?
          </h4>
    ...
    <div className="glass rounded-[2.5rem] overflow-hidden">
      <div
        className={`p-6 border-b ${
          isDark
            ? 'border-white/5 bg-gradient-to-r from-[var(--accent-color)]/10 to-transparent'
            : `${feature.bgLight} border-slate-100`
        }`}
      >
        <div className="flex items-center gap-3 mb-3">
          <div
            className={`${feature.color} w-12 h-12 rounded-xl flex items-center justify-center shadow-lg`}
          >
            {(() => {
      ...
      <div className="p-6 min-h-screen overflow-y-auto">{children}</div>
    </div>
  );
}
