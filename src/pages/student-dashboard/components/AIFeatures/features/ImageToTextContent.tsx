...
export function ImageToTextContent() {
  ...
  return (
    <div className="space-y-6">
      <div className={`${isDark ? 'bg-teal-500/10 border-teal-500/20' : 'bg-gradient-to-br from-teal-50 to-teal-100 border-teal-200'} rounded-xl p-6 border`}>
      ...
      <div className={`border-2 border-dashed ${isDark ? 'border-white/20 bg-card-dark' : 'border-slate-200 bg-gradient-to-b from-white to-background-light'} rounded-xl p-10 text-center hover:border-teal-500 transition-all cursor-pointer group`}>
        ...
        <h4 className={`${isDark ? 'text-white' : 'text-slate-800'} mb-2`}>Upload Image or Screenshot</h4>
        <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'} mb-4`}>
          Drop files here or click to select
        </p>
        <div className={`inline-flex items-center gap-2 text-xs text-teal-700 ${isDark ? 'bg-white/5' : 'bg-white'} px-4 py-2 rounded-lg`}>
      ...
      <div>
        <label className={`block text-sm ${isDark ? 'text-slate-300' : 'text-slate-700'} mb-3`}>Processing Options</label>
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <label key={idx} className={`flex items-center gap-3 p-3 border-2 ${isDark ? 'border-white/10 hover:bg-white/5' : 'border-slate-100 hover:bg-slate-50'} rounded-lg cursor-pointer transition-all`}>
              <input 
                type="checkbox" 
                className="w-4 h-4 text-teal-600 rounded focus:ring-teal-500" 
                defaultChecked={idx === 0}
              />
              <span className={`text-sm ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{option}</span>
            </label>
          ))}
        </div>
      </div>
