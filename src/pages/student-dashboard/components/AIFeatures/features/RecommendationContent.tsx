...
export function RecommendationContent() {
  ...
  return (
    <div className="space-y-5">
      <div
        className={`${isDark ? 'bg-amber-500/10 border-amber-500/20' : 'bg-gradient-to-br from-amber-50 to-amber-100 border-amber-200'} rounded-xl p-6 border`}
      >
      ...
      <div>
        <h5
          className={`${isDark ? 'text-white' : 'text-slate-800'} mb-3 flex items-center justify-between`}
        >
            ...
            >
              <div className="flex items-start justify-between mb-2">
                <div className="flex-1">
                  <h6
                    className={`${isDark ? 'text-white' : 'text-slate-800'} mb-1 group-hover:text-amber-700 transition-colors`}
                  >
                    {item.title}
                  </h6>
                  <div
                    className={`flex items-center gap-3 text-xs ${isDark ? 'text-slate-400' : 'text-slate-600'}`}
                  >
                    <span className={`${isDark ? 'bg-white/5' : 'bg-slate-50'} px-2 py-1 rounded`}>
                      {item.type}
                    </span>
      ...
      >
        <div
          className={`flex items-center gap-2 text-sm ${isDark ? 'text-slate-300' : 'text-slate-700'} mb-2`}
        >
