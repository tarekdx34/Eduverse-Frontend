...
export function FeedbackContent() {
  ...
  return (
    <div className="space-y-5">
      <div className={`${isDark ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-gradient-to-br from-emerald-50 to-emerald-100 border-emerald-200'} rounded-xl p-6 border`}>
        <h4 className={`${isDark ? 'text-white' : 'text-emerald-900'} mb-2 flex items-center gap-2`}>
      ...
      <div>
        <label className={`block text-sm ${isDark ? 'text-slate-300' : 'text-slate-700'} mb-3`}>Paste Your Text Below</label>
        <textarea
          rows={10}
      ...
      <div>
        <label className={`block text-sm ${isDark ? 'text-slate-300' : 'text-slate-700'} mb-3`}>Analysis Preferences</label>
