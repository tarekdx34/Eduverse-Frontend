...
export function SummarizerContent() {
  ...
  return (
    <div className="space-y-6">
      <div
        className={`${isDark ? 'bg-blue-500/10 border-blue-500/20' : 'bg-gradient-to-br from-blue-50 to-blue-100 border-blue-300'} border-2 border-dashed rounded-xl p-10 text-center hover:border-blue-500 transition-all cursor-pointer group`}
      >
        <div
        ...
        <div
          className={`inline-flex items-center gap-2 text-xs text-blue-700 ${isDark ? 'bg-white/5' : 'bg-white'} px-4 py-2 rounded-lg`}
        >
      ...
      <div className="space-y-3">
        <label className={`block text-sm ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
          Summary Length
        </label>
      ...
      <div className="flex gap-3">
        ...
        <button
          className={`px-6 py-3 border-2 ${isDark ? 'border-white/10 text-slate-300 hover:bg-white/5' : 'border-slate-100 text-slate-700 hover:bg-slate-50'} rounded-lg transition-all`}
        >
          Clear
        </button>
      </div>
      ...
      <div
        className={`${isDark ? 'bg-card-dark border-white/5' : 'bg-background-light border-slate-100'} rounded-lg p-4 border`}
      >
        <div
          className={`flex items-center gap-2 text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'} mb-2`}
        >
        ...
        <div className="space-y-2">
          <div
            className={`${isDark ? 'bg-white/5 text-slate-300' : 'bg-white text-slate-700'} rounded p-3 text-sm hover:shadow-sm cursor-pointer transition-shadow`}
          >
