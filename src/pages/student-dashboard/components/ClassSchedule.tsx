    ...
    <div className="space-y-8" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-8">
        ...
        <div className="lg:col-span-2">
          {/* Header */}
          <div
            className={`rounded-[2.5rem] p-6 mb-6 ${isDark ? 'bg-card-dark border border-white/5' : 'glass'}`}
          >
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className={`p-2 border-2 rounded-lg transition-all ${isDark ? 'border-white/10 hover:bg-white/5' : 'border-slate-100 hover:bg-slate-50'}`}
                >
                  <ChevronLeft
            ...
            <div className="overflow-x-auto">
              ...
              {scheduleType === 'weekly' && (
                <>
                  {/* Week Days Header */}
                  <div
                    className={`grid grid-cols-8 min-w-[700px] border-b ${isDark ? 'border-white/5' : 'border-slate-100'}`}
                  >
                    <div
                  ...
                  <div className="overflow-y-auto max-h-[600px] scrollbar-thin scrollbar-thumb-blue-500/30 scrollbar-track-transparent">
                    {timeSlots.map((time) => (
                      <div
                        key={time}
                        className={`grid grid-cols-8 min-w-[700px] border-b last:border-b-0 transition-colors ${isDark ? 'border-white/5 hover:bg-white/5/30' : 'border-slate-100 hover:bg-slate-50/50'}`}
                      >
                        <div
                        ...
                        {weekDays.map((day) => {
                          ...
                          return (
                            <div
                              key={`${day}-${time}`}
                              className={`p-2 border-r last:border-r-0 min-h-[80px] relative ${isDark ? 'border-white/5' : 'border-slate-100'}`}
                            >
                              {dayClasses.map((classItem) => (
                                ...
                                >
                                  <p
                                    className={`text-xs font-bold ${classItem.textColor} mb-1 group-hover:opacity-80`}
                                  >
                                    {classItem.code}
                                  </p>
                                  <p
                  ...
                  <div className="space-y-4">
                    {timeSlots.map((time) => {
                      ...
                      return (
                        <div
                          key={time}
                          className={`flex gap-4 ${isDark ? 'border-white/5' : 'border-slate-100'}`}
                        >
          ...
          >
            <div className="mb-6">
              <h3
                className={`text-lg font-semibold mb-1 ${isDark ? 'text-white' : 'text-slate-800'}`}
              >
                {isRTL ? 'المحاضرات القادمة' : 'Upcoming Classes'}
              </h3>
              <p className={`text-sm ${isDark ? 'text-slate-500' : 'text-slate-600'}`}>
                {isRTL ? 'محاضراتك المجدولة القادمة' : 'Your next scheduled classes'}
              </p>
            </div>
              ...
              {upcomingClasses.map((classItem) => (
                ...
                >
                  <div className="flex items-start gap-3 mb-3">
                    <div
                      className={`w-1.5 h-20 ${classItem.color} rounded-full flex-shrink-0`}
                    ></div>
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Progress */}
                      <div
                        className={`mt-3 pt-3 border-t ${isDark ? 'border-white/5' : 'border-slate-100'}`}
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span
                            className={`text-xs font-bold px-2 py-0.5 rounded ${isDark ? 'text-[var(--accent-color)]/70 bg-[var(--accent-color)]/20' : 'text-[var(--accent-color)] bg-[var(--accent-color)]/10'}`}
                          >
                            {classItem.progress}%
                          </span>
                        </div>
