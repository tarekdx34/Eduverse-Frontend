...
export function SecurityPage({
  ...
  const [activeTab, setActiveTab] = useState<
    'events' | 'certificates' | 'access' | 'policies' | 'ai'
  >('events');
              ...
              <tbody className={`divide-y ${isDark ? 'divide-gray-700' : 'divide-gray-200'}`}>
                {sslCertificates.map((cert) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`px-2 py-1 rounded text-xs font-medium ${getCertStatusColor(cert.status)}`}
                      >
                        {t(cert.status)}
                      </span>
                    </td>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <input
                      type="number"
                      defaultValue={90}
                      className={`w-20 px-2 py-1 rounded border text-sm ${isDark ? 'bg-gray-600 border-gray-500 text-white' : 'bg-white border-gray-300 text-gray-900'}`}
                    />
                  </div>
                </div>
                <button className="mt-4 px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:opacity-90 transition-colors">
                  Save
                </button>
              </div>
              ...
              {[
                {
                  ...
                  icon: '🔴',
                  text: 'Critical: 3 accounts with weak passwords detected. Force password reset recommended.',
                },
                {
                  id: 2,
                  severity: 'warning',
                  ...
                  icon: '🟡',
                  text: 'Warning: Unusual login pattern detected for dr.smith@uni.edu - 15 logins from 5 different IPs in 24h.',
                },
                {
                  id: 3,
                  severity: 'info',
                  ...
                  icon: '🔵',
                  text: 'Info: SSL certificate for api.university.edu expires in 30 days. Auto-renewal is configured.',
                },
                {
                  id: 4,
                  severity: 'positive',
                  ...
                  icon: '🟢',
                  text: 'Positive: No brute force attempts detected in the last 7 days. Security posture improving.',
                },
              ].map((insight) => (
                <div
                  key={insight.id}
                ...
                >
                  <Sparkles
                    className={`w-5 h-5 mt-0.5 ${isDark ? 'text-blue-400' : 'text-blue-600'}`}
                  />
                  <div className="flex-1">
                  ...
                  <div className="flex gap-2 shrink-0">
                    ...
                    <button
                      className={`px-3 py-1.5 text-xs rounded-lg transition-colors ${isDark ? 'bg-gray-600 text-gray-300 hover:bg-gray-500' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                    >
                      Dismiss
                    </button>
                  </div>
