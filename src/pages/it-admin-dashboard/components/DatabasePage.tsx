...
export function DatabasePage({
  ...
  const [activeTab, setActiveTab] = useState<
    'backups' | 'dr-runbooks' | 'integrity' | 'ai-recommendations'
  >('backups');
  ...
  const integrityChecks = [
    {
      ...
      details: 'Tables: 45/45 passed • Indexes: 120/120 valid • FK constraints: 85/85 valid',
      healthy: true,
    },
    {
      name: 'MongoDB Analytics',
      details: 'Collections: 12/12 passed • Indexes: 28/28 valid',
      healthy: true,
    },
    {
      name: 'Redis Cache',
      details: 'Keys: 15,200 • Memory: 2.1GB/4GB • Evictions: 0',
      healthy: true,
    },
  ...
  const aiRecommendations = [
    {
      text: 'Consider adding a read replica for PostgreSQL - query load increased 25% this month',
      type: 'info' as const,
    },
    {
      text: 'Redis memory usage at 52%. Current growth rate suggests upgrade needed in ~3 months',
      type: 'warning' as const,
    },
    {
      text: "Elasticsearch index 'access_logs' hasn't been optimized in 45 days. Force merge recommended",
      type: 'caution' as const,
    },
    {
      text: 'Backup retention of 30 days is optimal for current data growth rate. No changes needed.',
  ...
  const getRecommendationStyle = (type: 'info' | 'warning' | 'caution' | 'positive') => {
    switch (type) {
      case 'info':
        return isDark ? 'border-blue-500/50 bg-blue-500/10' : 'border-blue-300 bg-blue-50';
      case 'warning':
        return isDark ? 'border-yellow-500/50 bg-yellow-500/10' : 'border-yellow-300 bg-yellow-50';
      case 'caution':
        return isDark ? 'border-orange-500/50 bg-orange-500/10' : 'border-orange-300 bg-orange-50';
      case 'positive':
        return isDark ? 'border-green-500/50 bg-green-500/10' : 'border-green-300 bg-green-50';
    }
  };
                ...
                <tbody className={`divide-y ${isDark ? 'divide-gray-700' : 'divide-gray-200'}`}>
                  {backups.map((backup) => (
                    ...
                    >
                      ...
                      <td
                        className={`px-6 py-4 whitespace-nowrap ${isDark ? 'text-gray-300' : 'text-gray-700'}`}
                      >
                        {backup.schedule}
                      </td>
                      <td
                        className={`px-6 py-4 whitespace-nowrap ${isDark ? 'text-gray-400' : 'text-gray-500'}`}
                      >
                        {backup.lastRun}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`px-2 py-1 rounded text-xs font-medium ${getStatusColor(backup.status)}`}
                        >
                          {backup.status}
                        </span>
                      </td>
            ...
            >
              ...
              <div className="flex gap-4 mb-4">
                <div className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                  <span className={`${isDark ? 'text-gray-400' : 'text-gray-500'}`}>RTO:</span>{' '}
                  {runbook.rto}
                </div>
                <div className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                  <span className={`${isDark ? 'text-gray-400' : 'text-gray-500'}`}>RPO:</span>{' '}
                  {runbook.rpo}
                </div>
              </div>
      ...
      {activeTab === 'ai-recommendations' && (
        <div className="space-y-4">
          {aiRecommendations.map((rec, index) => (
            <div
              key={index}
              className={`rounded-xl border p-5 ${getRecommendationStyle(rec.type)}`}
            >
              <div className="flex items-start gap-4">
                <Sparkles
                  className={`w-5 h-5 mt-0.5 flex-shrink-0 ${getRecommendationIcon(rec.type)}`}
                />
                <div className="flex-1">
