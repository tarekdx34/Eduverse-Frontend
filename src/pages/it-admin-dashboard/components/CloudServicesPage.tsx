...
export function CloudServicesPage() {
  ...
  const filteredServices =
    activeProvider === 'All' ? services : services.filter((s) => s.provider === activeProvider);

  const budget = 5000;
  const spent = 4280;
  const budgetPercent = (spent / budget) * 100;
  const budgetColor =
    budgetPercent > 95 ? 'bg-red-500' : budgetPercent > 80 ? 'bg-yellow-500' : 'bg-green-500';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className={`mt-1 ${textSecondary}`}>
            Manage and monitor cloud infrastructure across providers
          </p>
        </div>
      ...
      <div className={cardClass}>
        ...
        <div className="flex items-center justify-center gap-6 mt-4">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-sm ${textSecondary}`}>AWS</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-sm bg-blue-500" />{' '}
            <span className={`text-sm ${textSecondary}`}>Azure</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-sm bg-red-500" />{' '}
            <span className={`text-sm ${textSecondary}`}>GCP</span>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-end justify-between mb-2">
          <div>
            ...
            <p className={`text-2xl font-bold ${textPrimary}`}>
              ...
              <span className={`text-base font-normal ${textSecondary}`}>
                / ${budget.toLocaleString()}
              </span>
            </p>
          </div>
          <span
            className={`text-sm font-medium ${budgetPercent > 95 ? 'text-red-500' : budgetPercent > 80 ? 'text-yellow-500' : 'text-green-500'}`}
          >
      ...
      <div className={cardClass}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {regions.map((region) => (
            <div
              key={region.name}
              className={`rounded-lg border p-4 ${isDark ? 'border-gray-700 bg-gray-750' : 'border-gray-200 bg-gray-50'}`}
            >
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`text-sm font-medium ${textPrimary}`}>
                    ${region.cost.toLocaleString()}/mo
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className={`text-sm ${textSecondary}`}>Latency</span>
                  <span
                    className={`text-sm font-medium ${region.latency > 100 ? 'text-yellow-500' : isDark ? 'text-green-400' : 'text-green-600'}`}
                  >
                    {region.latency}ms
                  </span>
                </div>
