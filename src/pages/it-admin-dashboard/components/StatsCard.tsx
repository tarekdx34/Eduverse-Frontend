...
export function StatsCard({ label, value, comparison, isPositive, icon }: StatsCardProps) {
  ...
  return (
    <div
      className={`rounded-xl p-6 border shadow-sm ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}
    >
      ...
      {comparison && (
        <div className="flex items-center justify-between">
          ...
          <div
            className={`px-3 py-1 rounded text-sm font-medium ${isPositive ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}
          >
            {comparison}
          </div>
        </div>
