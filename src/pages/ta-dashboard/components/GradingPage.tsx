      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className={`mt-1 ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>
            Review live lab submissions and update their grading status.
          </p>
        </div>
        ...
        <>
          <div className={`border rounded-lg p-4 ${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'}`}>
            <div className="flex flex-wrap gap-2">
              {(['all', 'submitted', 'graded', 'returned', 'resubmit'] as const).map((status) => (
                ...
                >
                  {status === 'all' ? 'All' : statusCopy[status]}
                </button>
              ))}
            </div>
          </div>
              ...
              >
                ...
                {submission.submissionText && (
                  <div className={`mb-4 p-3 border rounded-lg ${isDark ? 'bg-white/5 border-white/10' : 'bg-gray-50 border-gray-200'}`}>
                    <h4 className={`text-sm font-medium mb-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>Submission Notes</h4>
          ...
          {filteredSubmissions.length === 0 && (
            <div className={`text-center py-12 border rounded-lg ${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'}`}>
              ...
              <p className={isDark ? 'text-slate-400' : 'text-gray-600'}>No live submissions found.</p>
            </div>
          )}
        </>
      ) : (
        <div className={`border rounded-lg p-6 ${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'}`}>
          <div className={`mb-4 rounded-lg border px-4 py-3 text-sm ${isDark ? 'border-amber-500/30 bg-amber-500/10 text-amber-200' : 'border-amber-200 bg-amber-50 text-amber-900'}`}>
            Auto-grading is not connected yet. This panel currently shows only submissions already marked as graded.
          </div>
          {autoResults.length > 0 ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className={isDark ? 'bg-white/5' : 'bg-gray-50'}>
                    <th className={`text-left px-6 py-3 font-medium ${isDark ? 'text-slate-300' : 'text-gray-700'}`}>Student Name</th>
                    <th className={`text-left px-6 py-3 font-medium ${isDark ? 'text-slate-300' : 'text-gray-700'}`}>Lab</th>
                    <th className={`text-left px-6 py-3 font-medium ${isDark ? 'text-slate-300' : 'text-gray-700'}`}>Status</th>
                    <th className={`text-left px-6 py-3 font-medium ${isDark ? 'text-slate-300' : 'text-gray-700'}`}>Submitted</th>
                  </tr>
                </thead>
                <tbody>
                  {autoResults.map((result) => (
                    <tr key={result.id} className={`border-t ${isDark ? 'border-white/10 hover:bg-white/5' : 'border-gray-100 hover:bg-gray-50'}`}>
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${statusClasses.graded}`}>{result.status}</span>
                      </td>
            ...
            <p className={isDark ? 'text-slate-400' : 'text-gray-600'}>No graded submissions yet.</p>
          )}
        </div>
      )}
          ...
          >
            ...
            <div className="space-y-5 px-6 py-5">
              <div className="grid gap-4 md:grid-cols-2">
                <div className={`rounded-xl border p-4 ${isDark ? 'border-white/10 bg-white/5' : 'border-gray-200 bg-gray-50'}`}>
                  <p className={`text-xs font-medium uppercase tracking-wide ${isDark ? 'text-slate-400' : 'text-gray-500'}`}>Submitted At</p>
                  <p className="mt-2 text-sm">{new Date(selectedSubmission.submittedAt).toLocaleString()}</p>
                </div>
                <div className={`rounded-xl border p-4 ${isDark ? 'border-white/10 bg-white/5' : 'border-gray-200 bg-gray-50'}`}>
                  <p className={`text-xs font-medium uppercase tracking-wide ${isDark ? 'text-slate-400' : 'text-gray-500'}`}>Current Status</p>
                  <p className="mt-2">
                    <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${statusClasses[selectedSubmission.status]}`}>
                      {statusCopy[selectedSubmission.status]}
                    </span>
                  </p>
                </div>
              </div>
              ...
              <div>
                <label className={`mb-2 block text-sm font-medium ${isDark ? 'text-slate-200' : 'text-gray-800'}`}>Submission Notes</label>
                <div className={`min-h-28 rounded-xl border p-4 text-sm ${isDark ? 'border-white/10 bg-white/5 text-slate-200' : 'border-gray-200 bg-gray-50 text-gray-700'}`}>
                  {selectedSubmission.submissionText?.trim() || 'No text notes were submitted.'}
                </div>
              </div>

              <div>
                <label className={`mb-2 block text-sm font-medium ${isDark ? 'text-slate-200' : 'text-gray-800'}`}>Set Status</label>
                <select
            ...
            <div className="flex items-center justify-end gap-3 border-t border-inherit px-6 py-4">
              ...
              >
                ...
                {gradingSubmissionId ? 'Saving...' : 'Save Status'}
              </button>
            </div>
          </div>
        </div>
