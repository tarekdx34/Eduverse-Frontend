...
export function StudentPerformancePage({ students }: StudentPerformancePageProps) {
  ...
  const getPerformanceStatus = (average: number) => {
    if (average >= 90) return { label: t('excellent'), color: isDark ? 'text-green-400' : 'text-green-600', bg: isDark ? 'bg-green-500/20' : 'bg-green-50' };
    if (average >= 80) return { label: t('good'), color: isDark ? 'text-blue-400' : 'text-blue-600', bg: isDark ? 'bg-blue-500/20' : 'bg-blue-50' };
    if (average >= 70) return { label: t('fair'), color: isDark ? 'text-yellow-400' : 'text-yellow-600', bg: isDark ? 'bg-yellow-500/20' : 'bg-yellow-50' };
    return { label: t('needsImprovement'), color: isDark ? 'text-red-400' : 'text-red-600', bg: isDark ? 'bg-red-500/20' : 'bg-red-50' };
  };
          ...
          {student ? (
            <>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className={`${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} border rounded-lg p-4`}>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className={`text-sm ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>{t('overallAverage')}</span>
                  </div>
                ...
                <div className={`${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} border rounded-lg p-4`}>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className={`text-sm ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>{t('overallAttendance')}</span>
                  </div>
              ...
              <div className={`${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} border rounded-lg p-6`}>
                <h3 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'} mb-4`}>{t('performanceByCourse')}</h3>
              ...
              {/* Course Details */}
              <div className={`${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} border rounded-lg p-6`}>
                <h3 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'} mb-4`}>{t('courseDetails')}</h3>
                <div className="space-y-4">
                  {student.courses.map((course) => (
                    <div key={course.courseId} className={`border ${isDark ? 'border-white/10' : 'border-gray-200'} rounded-lg p-4`}>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div>
                          <span className={`text-xs ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>{t('averageGrade')}</span>
                          <p className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>{course.averageGrade}%</p>
                        </div>
                        <div>
                          <span className={`text-xs ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>{t('attendance')}</span>
                          <p className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>{course.attendanceRate}%</p>
                        </div>
                        <div>
                          <span className={`text-xs ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>{t('submissions')}</span>
                          <p className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>{course.submissionCount}</p>
                        </div>
                        <div>
                          <span className={`text-xs ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>{t('late')}</span>
            ...
            <div className={`${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} border rounded-lg p-12 text-center`}>
              ...
              <p className={`${isDark ? 'text-slate-400' : 'text-gray-600'}`}>{t('selectStudentMessage')}</p>
            </div>
          )}
        </div>
      </div>
